...
app.controller('rpmacController', ["$scope", "$interval", "socket", function($scope, $interval, socket) {
  var rpmacc = this;
...
angular.module("adf.widget.rpmaccel").run(["$templateCache", function($templateCache) {$templateCache.put("{widgetsPath}/rpmaccel/src/edit.html","<form role=form><div class=form-group></div></form>");
$templateCache.put("{widgetsPath}/rpmaccel/src/view.html","<div ng-controller=rpmacController><div ng-if=!chartConfig><h1>Loading...</h1></div><div ng-if=chartConfig><highchart id=chart1 config=chartConfig></highchart></div></div>");}]);})(window);